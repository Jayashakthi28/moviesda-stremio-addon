...
const getGoogleStreams = (movieData) => {
    if (!movieData.ready_streams) return [];
            return movieData?.ready_streams.filter(source => source.source === 'google_drive').map(source => ({
                url: source.url,
                name: `MoviesDA⚡️\n\n[Tamil]`,
...
const getTeraBoxDirectDownloadUrl = async(link) => {
...
const getTeraBoxStreams = async(movieData) =>{
    if(!movieData.ready_streams) return [];
    const terraboxSources = movieData?.ready_streams.filter(source => source.source === 'terrabox');

    const terraboxData = [];
    for(const source of terraboxSources){
        const url = await getTeraBoxDirectDownloadUrl(source.url);
        if(url){
            terraboxData.push({
                url: url,
                name: `MoviesDA⚡️\n\n[Tamil]`,
...
const getReadyStreams = async(movieData) => {
    if (!movieData.ready_streams) return [];

    const googleStreams = getGoogleStreams(movieData);
    console.log(googleStreams);
    const terraboxStreams = await getTeraBoxStreams(movieData);

    console.log(terraboxStreams);

    return [...googleStreams, ...terraboxStreams];
}
...
async function scrapeStreamUrl(link) {
    ...
    // Skip if filename contains "sample" (case-insensitive)
    if (fileName.toLowerCase().includes('sample')) {
        console.log(`⚠️  Skipping sample file: ${fileName}`);
        return [];
    }
    ...
    if (downloadElement) {
        ...
        for (const linkEl of links) {
            ...
            if (href) {
                // Encode URL properly using built-in encodeURI
...
module.exports = { findMovieInDB, scrapeAllStreams, getReadyStreams };
